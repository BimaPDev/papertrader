...
| `MONITOR_ALERT_KINDS` / `MONITOR_SWARM_KINDS` | `almost`, `migrated`, dex, copy | which stages alert + get graded |
| `MONITOR_PAPER_MAX_POSITIONS` | `8` | sniper open-position cap (separate from hourly `MAX_OPEN_POSITIONS`) |
| `MONITOR_SWARM_RESCORE_ON_MIGRATE` | `True` | re-swarm once when Almost → Migrated |
